#!/bin/bash
# Deploy SQLite Database Implementation
# Run this on your droplet: bash deploy_sqlite.sh

set -e

REPO_DIR="/root/clawdbot_polymarket_trader"

echo "🦀 Deploying SQLite Database Implementation..."
echo ""

# Navigate to repo
cd "$REPO_DIR"

# Create src/utils/sqlite_database.ts
echo "📝 Creating src/utils/sqlite_database.ts..."
cat > src/utils/sqlite_database.ts << 'SQLITEFILE'
/**
...
echo "✅ Created sqlite_database.ts"
echo ""

# Update src/web/server.ts imports
echo "📝 Updating src/web/server.ts imports..."

# Backup original
cp src/web/server.ts src/web/server.ts.backup

# Update imports (lines 7-9)
sed -i "10s|import { getRecentTrades, getWhaleTrades } from '../api/trade_feed';|import { getRecentTrades as fetchRecentTrades } from '../api/trade_feed';|" src/web/server.ts
sed -i "11s|import { storeTrades, readTrades } from '../utils/trade_database';|import { \n  storeTrades, \n  readTrades, \n  getWhaleTrades,\n  getDatabaseStats,\n  getDatabaseSize,\n  migrateFromJSON \n} from '../utils/sqlite_database';|" src/web/server.ts

echo "⚠️  Import replacement may need manual verification"
echo "   Backup saved to src/web/server.ts.backup"
echo ""
...
if [ $? -eq 0 ]; then
  ...
  echo ""
  echo "📊 Check dashboard at: http://174.138.55.80:3000"
  echo "📈 Check stats at: http://174.138.55.80:3000/stats"
else
